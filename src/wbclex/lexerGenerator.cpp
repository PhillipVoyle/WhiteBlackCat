...
void CLexerGenerator::ComputeStates(std::vector<PositionSet> followPos, std::shared_ptr<CRegularExpression> exp, std::vector<StateData>& stateData)
{
	...
	while(nMarkedStates < nStates)
	{
		...
		for(int a = 0; a < nSymbols; a++)
		{
			...
			std::map<std::vector<int>, int>::iterator itState = DStateIDs.find(U);
			int stateID;
...
{
	std::map<std::string, std::string>::iterator it = m_vars.find(vName);
	if(it == m_vars.end())
	{
		return false;
	}
...
{
	s << "/*" << std::endl;
	s << "    " << fname << "\n    Generated by Lexigen Scanner Generator (C) Phillip Voyle 2011" << std::endl;
	s << "*/" << std::endl;
	s << std::endl;
}
...
void CLexerGenerator::Export()
{
  ...
  PrintBanner(fTableHeader, sTableHeader);
	...
	fTableHeader << "   {\n";
	int nNumStates = 0;
	STATEDATAVECTORMAP::iterator itDVM;
  ...
  fHeader << "            assert(c >= 0);\n";
	...
	for(itDVM = m_stateData.begin(); itDVM != m_stateData.end(); itDVM++)
	{
		...
		STATEDATAVECTOR& dv = itDVM->second;
		std::map<std::string, int>::iterator itEOF = m_eofEvents.find(itDVM->first);
